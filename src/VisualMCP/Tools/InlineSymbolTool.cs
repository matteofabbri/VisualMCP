...
public static class InlineSymbolTool
{
    ...
    public static async Task<object> InlineSymbol(
        ...
        [Description("Dry run — show what would change without writing to disk (default: false)")] bool dryRun = false)
    {
        var solution = RoslynWorkspaceService.Instance.CurrentSolution;
        if (solution is null)
            return new { error = "No solution loaded. Call load_solution first." };

        // ── Try member (method / property) first ───────────────────────────
        ...
        // ── Fall back: look for local variable across all documents ─────────
    ...
    private static async Task<object> InlineMember(ISymbol symbol, Solution solution, bool dryRun)
    {
        ...
        if (allLocs.Count == 0)
            return new { error = "Symbol has no references — nothing to inline into." };
            ...
            foreach (var document in project.Documents.Where(d => d.SourceCodeKind == SourceCodeKind.Regular))
            {
                ...
                foreach (var localDecl in locals)
                {
                    ...
                    if (variable.Initializer is null)
                        return new { error = $"Local variable '{symbolName}' has no initializer — cannot inline." };
                    ...
                    // Check for writes (assignments) — refuse if the variable is reassigned
                    ...
                    if (assignments.Count > 0)
                        return new { error = $"Local variable '{symbolName}' is reassigned — cannot safely inline." };
                    ...
                    // but the usages may be after the declaration — so the declaration span is stable)
