...
{
    [McpServerTool, Description("Delete a symbol (type, method, property, field) from source only if it has zero references in the solution. Refuses and lists all references if any exist — equivalent to ReSharper Safe Delete. Requires LoadSolution first.")]
    public static async Task<object> SafeDelete(
        ...
        [Description("Dry run — verify safety without deleting (default: false)")] bool dryRun = false)
    {
